 ...
 *  12345, 6    => false
 */
function isContainNumber(num, d) {
...
function rotateMatrix(matrix) {
  ...
  for (let row = 0; row < size; row += 1) {
    for (let col = 0; col < size; col += 1) {
      auxMatrix[size - col - 1][size - row - 1] = copyMatrix[row][col];
    }
  }

  let j = size - 1;

  for (let i = 0; i < auxMatrix.length; i += 1) {
    copyMatrix[i] = auxMatrix[j];
    j -= 1;
  }
...
function heapify(arr, length, i) {
  ...
  if (left < length && copyArr[left] > copyArr[large]) {
    large = left;
  }

  if (right < length && copyArr[right] > copyArr[large]) {
    large = right;
  }

  if (large !== i) {
    const swap = copyArr[i];
    copyArr[i] = copyArr[large];
    copyArr[large] = swap;
...
function sort(arr) {
  const copyArr = arr;
  for (let i = Math.floor(copyArr.length / 2) - 1; i >= 0; i -= 1) {
    heapify(copyArr, copyArr.length, i);
  }
...
function getNearestBigger(number) {
  ...
  result += `${left.join('')}${right.sort().join('')}`;

  return Number(result) > number ? Number(result) : number;
}
